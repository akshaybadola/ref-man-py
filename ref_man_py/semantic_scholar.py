...
class SemanticScholar:
    ...
    def __init__(self, config_file=None, cache_dir=None):
        ...
        self._root_url = "https://api.semanticscholar.org/graph/v1/paper"
    ...
    def details_url(self, ID: str):
        ...
        return f"{self._root_url}/{ID}?fields={fields}"

    def citations_url(self, ID: str):
        ...
        return f"{self._root_url}/{ID}/citations?fields={fields}&limit={limit}"

    def references_url(self, ID: str):
        ...
        return f"{self._root_url}/{ID}/references?fields={fields}&limit={limit}"
    ...
    async def _paper(self, ID: str) -> Dict[str, Dict]:
        ...
        """
        urls = [f(ID) for f in [self.details_url, self.references_url, self.citations_url]]
        if self._api_key:
    ...
    def _citations(self, ID: str, num: int = 0):
        ...
        url = f"{self._root_url}/{ID}/citations?fields={fields}&limit={limit}"
    ...
    def _references(self, ID: str, num: int = 0):
        ...
        url = f"{self._root_url}/{ID}/references?fields={fields}&limit={limit}"
        return self._get(url)
    ...
    def next_citations(self, ID: str, num: int = 0) -> Optional[Dict]:
        ...
        if data is not None and "next" in data["citations"]:
            ...
            url = f"{self._root_url}/{ID}/citations?fields={fields}&limit={limit}&offset={offset}"
